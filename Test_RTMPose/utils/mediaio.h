...
class Output {
 public:
  explicit Output(const std::string& path, int show, double fps = 30.0, MediaType type = MediaType::kUnknown)
      : path_(path), type_(type), show_(show), fps_(fps) {
 ...
 private:
  ...
  void open_video(const cv::Size& size) { video_.open(path_, detail::ext2fourcc(ext_), fps_, size); }
